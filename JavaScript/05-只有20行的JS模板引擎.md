...
`code`变量保存了函数体。它从数组的定义开始。如前所述，`cursor`显示了我们当前解析到了模板中的哪个位置。我们需要这样一个变量来遍历整个模板字符串并跳过数据块。创建一个额外的`add`函数。它的工作是向code变量追加代码行。不过这里有个棘手的问题，那就是我们需要转义双引号，否则生成的脚本将会无效。如果我们运行这个例子并查看控制台，我们将看到：
